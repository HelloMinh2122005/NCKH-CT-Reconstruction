...
from baselines.DuDoNet.models import DuDoNet_LA


def parse_args():
    parser = argparse.ArgumentParser(description="Huấn luyện mô hình Baseline DuDoNet trên Limited-Angle CT")
    ...
    parser.add_argument("--sino_loss_weight", type=float, default=0.1, help="Trọng số mất mát miền sinogram gamma (mặc định: 0.1)")
    ...
    parser.add_argument("--default_root_dir", type=str, default="/datastore/uittogether3/LuuTru/MinhPD/scripts/output/train_dudonet_la/")
...
def main():
    ...
    if args.dataset_type == "aapm":
        ckpt_dir = os.path.join(args.saved_models_dir, "DuDoNet")
    else:
        ckpt_dir = os.path.join(args.saved_models_dir, args.dataset_type, "DuDoNet")
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename="dudonet_la-{epoch:02d}-{val_psnr:.2f}-{val_ssim:.4f}",
        monitor="val_psnr",
        mode="max",
        save_top_k=3,
        save_last=True,
    ...
    print(f"[INFO] Bắt đầu huấn luyện DuDoNet trên dải góc {args.angle_range_deg}°...")
