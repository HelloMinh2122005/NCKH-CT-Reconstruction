...
# Import DataModule Factory và Mô hình DuDoTrans
...
def parse_args():
    """
    Khai báo và phân tích toàn bộ tham số dòng lệnh phục vụ huấn luyện DuDoTrans trên Limited-Angle CT.
    """
    parser = argparse.ArgumentParser(description="Huấn luyện mô hình Baseline DuDoTrans (Dual-Domain) trên Limited-Angle CT")
    ...
    # 3. Cấu hình Mô hình DuDoTrans
    parser.add_argument(
        "--feature_dim",
        type=int,
        default=48,
        help="Số kênh đặc trưng trên cả hai miền Sinogram và Ảnh (mặc định = 48)"
    )
    parser.add_argument(
        "--lambda_sino",
        type=float,
        default=0.1,
        help="Hệ số phạt ràng buộc độ khớp dữ liệu miền chiếu sinogram (mặc định = 0.1)"
    )
    parser.add_argument(
        "--lr", "--initial_lr",
        dest="initial_lr",
    ...
    parser.add_argument(
        ...
        dest="output_dir",
        type=str,
        default="/datastore/uittogether3/LuuTru/MinhPD/saved_models/DuDoTrans/",
...
def main():
    ...
    print("=" * 80)
    print("🚀 BẮT ĐẦU HUẤN LUYỆN BASELINE: DuDoTrans (Dual-Domain Transformer) (Limited-Angle CT)")
    ...
    print(f"- Cấu hình Dual-Domain: feature_dim={args.feature_dim}, lambda_sino={args.lambda_sino}")
    ...
    experiment_name = f"DuDoTrans_{dataset_name_clean}_LA_{int(angle_range_deg)}deg_view{args.num_view}"
