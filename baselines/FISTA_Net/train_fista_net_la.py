...
from baselines.FISTA_Net.models import FISTA_Net_LA


def parse_args():
    parser = argparse.ArgumentParser(description="Huấn luyện mô hình Baseline FISTA-Net trên Limited-Angle CT")
    ...
    parser.add_argument("--n_iterations", type=int, default=10, help="Số stages unrolling của FISTA-Net (mặc định: 10)")
    parser.add_argument("--hidden_channels", type=int, default=32, help="Số kênh ẩn trong khối biến phân tích (mặc định: 32)")
    ...
    parser.add_argument("--default_root_dir", type=str, default="/datastore/uittogether3/LuuTru/MinhPD/scripts/output/train_fista_net_la/")
...
def main():
    ...
    if args.dataset_type == "aapm":
        ckpt_dir = os.path.join(args.saved_models_dir, "FISTA_Net")
    else:
        ckpt_dir = os.path.join(args.saved_models_dir, args.dataset_type, "FISTA_Net")
    ...
    # 2. Khởi tạo Mô hình FISTA_Net_LA
    model = FISTA_Net_LA(
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=ckpt_dir,
        filename="fista_net_la-{epoch:02d}-{val_psnr:.2f}-{val_ssim:.4f}",
        monitor="val_psnr",
        mode="max",
        save_top_k=3,
        save_last=True,
    ...
    print(f"[INFO] Bắt đầu huấn luyện FISTA-Net (K={args.n_iterations} stages) trên dải góc {args.angle_range_deg}°...")
