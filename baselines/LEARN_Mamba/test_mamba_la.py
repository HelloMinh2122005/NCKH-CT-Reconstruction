...
def parse_args():
    """
    Khai báo tham số dòng lệnh phục vụ đánh giá (Inference & Testing) mô hình LEARN_Mamba.
    """
    parser = argparse.ArgumentParser(description="Đánh giá mô hình Baseline LEARN_Mamba trên tập kiểm thử Limited-Angle CT")
    
    parser.add_argument("--checkpoint_path", type=str, required=True, help="Đường dẫn đến file trọng số .ckpt đã huấn luyện")
    parser.add_argument("--data_dir", type=str, default="dataset/limited_angle", help="Đường dẫn thư mục chứa dữ liệu .npy cache")
    parser.add_argument("--test_patients", nargs="+", default=["L310"], help="Danh sách mã bệnh nhân kiểm thử độc lập (mặc định: L310)")
...
def main():
    ...
    print("=" * 70)
    print("📊 TIẾN HÀNH ĐÁNH GIÁ MÔ HÌNH: LEARN_Mamba_LA")
    ...
    # 2. Khởi tạo DataModule cho tập test
    ...
    # 3. Chạy đánh giá qua PyTorch Lightning Trainer
    trainer = pl.Trainer(
    ...
    print("=" * 70)
    print("📈 KẾT QUẢ ĐO LƯỜNG ĐỘ CHÍNH XÁC TÁI TẠO (TEST RESULTS):")
