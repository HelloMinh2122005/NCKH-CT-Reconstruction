...
# Import DataModule Factory và Mô hình RegFormer
from data.datamodule_factory import get_datamodule
from baselines.RegFormer.models import RegFormer_LA



def parse_args():
    """
    Khai báo và phân tích toàn bộ tham số dòng lệnh phục vụ huấn luyện RegFormer trên Limited-Angle CT.
    """
    parser = argparse.ArgumentParser(description="Huấn luyện mô hình Baseline RegFormer trên Limited-Angle CT")
    ...
    # 3. Cấu hình Mô hình RegFormer
    parser.add_argument(
        "--n_iterations",
        type=int,
        default=14,
        help="Số giai đoạn unrolling của RegFormer (K = 14 stages)"
    )
    parser.add_argument(
        "--feature_dim",
        type=int,
        default=48,
        help="Số kênh đặc trưng của Local và Non-local branches (mặc định = 48)"
    )
    parser.add_argument(
        "--window_size",
        type=int,
    ...
    parser.add_argument(
        ...
        dest="output_dir",
        type=str,
        default="/datastore/uittogether3/LuuTru/MinhPD/saved_models/RegFormer/",
...
def main():
    ...
    print("=" * 80)
    print("🚀 BẮT ĐẦU HUẤN LUYỆN BASELINE: RegFormer (Local-Nonlocal Regularization) (Limited-Angle CT)")
    ...
    print(f"- Cấu hình Window Attention: window_size={args.window_size}x{args.window_size}, feature_dim={args.feature_dim}")
    ...
    experiment_name = f"RegFormer_{dataset_name_clean}_LA_{int(angle_range_deg)}deg_view{args.num_view}"
