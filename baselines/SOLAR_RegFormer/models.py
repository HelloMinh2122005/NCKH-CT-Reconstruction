"""
================================================================================
KIẾN TRÚC MÔ HÌNH: SOLAR_RegFormer (LIMITED-ANGLE CT RECONSTRUCTION)
...
class SafeCGSolver(nn.Module):
    ...
    1. Hoàn toàn không tạo hoặc lưu trữ ma trận Hessian (tiết kiệm hàng chục GB VRAM).
    2. Mỗi bước lặp CG chỉ thực hiện 1 lần chiếu thuận A(p) và 1 lần chiếu ngược A^T(.) trên GPU.
    3. Bảo đảm tính đối xứng xác định dương nghiêm ngặt (Strictly SPD) qua tham số hóa Softplus μ_t > 0,
       triệt tiêu hoàn toàn nguy cơ chia cho 0 hoặc phát sinh lỗi NaN.
    """
    ...
    def forward(
        ...
        for _ in range(self.cg_iters):
            ...
            # Damping 1e-7 chống lỗi số học chia cho 0
...
# =============================================================================
# 2. KHỐI TỰ CHÚ Ý DỰA TRÊN CỬA SỔ CỤC BỘ (WINDOW-BASED ATTENTION - W-MSA)
# =============================================================================
class WindowAttention(nn.Module):
    ...
    Thay vì tính ma trận tương quan chú ý trên toàn bộ N = H x W = 256 x 256 = 65.536 điểm ảnh
    (gây bùng nổ O(N^2) bộ nhớ và làm rò rỉ artifact vệt sọc ra toàn bộ ảnh), ảnh đặc trưng được
    chia thành các cửa sổ con kích thước M x M (ví dụ M = 8, tức mỗi cửa sổ có 64 điểm ảnh).
    Self-Attention chỉ được tính toán cục bộ bên trong từng cửa sổ:
        Attention(Q, K, V) = Softmax(Q K^T / sqrt(d) + B_rel) V
    trong đó B_rel là ma trận vị trí tương đối (Relative Position Bias) mã hóa hình học không gian 2D.
...
class SwinTransformerBlock(nn.Module):
    """
    Một khối Swin Transformer cho nhánh Phi Cục Bộ (Non-local Branch) của SOLAR-RegFormer.
    Bao gồm: LayerNorm -> WindowAttention (W-MSA) -> Residual -> LayerNorm -> MLP (GELU) -> Residual.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Ghép nối các cửa sổ trở lại không gian ảnh ban đầu (Merge Windows)
...
# =============================================================================
# 3. NHÁNH TRÍCH XUẤT ĐẶC TRƯNG CỤC BỘ (LOCAL CNN BRANCH)
# =============================================================================
class LocalCNNBranch(nn.Module):
    """
    Nhánh Trích Xuất Đặc Trưng Cục Bộ (Local Branch) trong khối Điều Hòa của SOLAR-RegFormer.
...
# =============================================================================
# 4. KHỐI ĐIỀU HÒA TIÊN NGHIỆM KÉP REGFORMER DUAL-BRANCH REGULARIZER
# =============================================================================
class RegFormerDualBranchRegularizer(nn.Module):
    """
    Khối Điều Hòa Tiên Nghiệm Kết Hợp Cục Bộ - Toàn Cục (Local-Nonlocal Regularizer):
    ...
    3. Non-Local Branch: Khối SwinTransformerBlock tính toán tương quan cửa sổ, triệt tiêu artifact vệt sọc sải dài.
    4. Feature Fusion: Ghép nối (Concatenation) hai luồng đặc trưng qua Conv 1x1 để điều phối tỷ trọng thông tin.
    5. Reconstruction Projection: Conv 3x3 chiếu về 1 kênh ảnh hiệu chỉnh gradient ∇R_t.
    """
    def __init__(self, in_channels: int = 1, out_channels: int = 1, feature_dim: int = 48, window_size: int = 8):
        ...
        # Nhánh Phi Cục Bộ (Non-Local Branch với Swin Transformer)
...
# =============================================================================
# 5. MÔ HÌNH TOÀN CỤC SOLAR_RegFormer_LA (PYTORCH LIGHTNING MODULE)
# =============================================================================
class SOLAR_RegFormer_LA(pl.LightningModule):
    """
    Mô hình Đề Xuất Đột Phá SOLAR-RegFormer hoàn chỉnh cho Limited-Angle CT.
    
    Đặc tính cốt lõi:
    1. Động cơ tối ưu hóa bậc 2 Matrix-Free Newton-CG (SafeCGSolver), loại bỏ dao động Zigzag.
    2. Khối điều hòa kép Local CNN + Swin Transformer giam hãm vệt sọc nêm khuyết trong cửa sổ 8x8.
    3. Chia sẻ trọng số tuần hoàn (Recurrent Weight Sharing): Toàn bộ 8 stages dùng chung duy nhất
       1 bộ điều hòa RegFormerDualBranchRegularizer, giảm 90% tham số (~0.28M params) chống Overfitting.
    4. Ràng buộc Strictly SPD bảo đảm độ ổn định số học tuyệt đối 100% không NaN.
    """
    def __init__(
        self,
        n_iterations: int = 8,
        ...
        # Bộ giải Conjugate Gradient bậc 2
    ...
    def forward(self, x_0: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        Luồng tính toán qua T stages unrolling bậc 2:
            x_{t+1} = Safe_CG_Solve((λ_t * A^T A + μ_t * I), b_t)
        với:
            b_t = λ_t * A^T(y) + μ_t * x_t - ∇R_t
        """
        x_t = x_0
    ...
    def on_train_epoch_end(self):
        # Giải phóng bộ nhớ đệm CUDA định kỳ nhằm tránh tràn bộ nhớ Texture của ASTRA Toolbox
    ...
    def radon_transform(
        ...
        """
        Cấu hình toán tử hình học Fan-Beam ODL/ASTRA chuẩn xác định trên toàn bộ hệ thống nghiên cứu:
        - src_radius: 600 mm, det_radius: 290 mm
        - Dải detector: [-480, 480] mm với 512 phần tử
        - Lưới giải phẫu: [-200, 200] mm với 256x256 pixel
        """
        xx = 200
