...
class LimitedAngleCT_Provider(Dataset):
    ...
    def __init__(
        ...
        paths = []
        if valid:
            # Thu thập các lát cắt CT độ dày 3mm, liều chuẩn (Full Dose - FD) cho tập validation
            ...
            # Thu thập các lát cắt CT cho tập test
            ...
            # Thu thập các lát cắt CT cho tập training
    ...
    def __getitem__(self, index):
        ...
        Trả về tuple: (đường dẫn slice, ảnh Ground Truth, ảnh FBP góc hạn chế/nhiễu, sinogram).
        """
        ...
        # -------------------------------------------------------------
            ...
            # 1. Đọc và hiệu chuẩn ảnh DICOM
            dcm = pydicom.read_file(slice_path)
            data_slice = dcm.pixel_array * dcm.RescaleSlope + dcm.RescaleIntercept
            data_slice = data_slice.astype(float)
            # Chuẩn hóa về [0, 1] để làm ảnh Ground Truth
            data_slice = (data_slice - np.min(data_slice)) / (np.max(data_slice) - np.min(data_slice) + 1e-8)
            phantom = torch.from_numpy(data_slice).unsqueeze(0).float()

            # 2. Chiếu tia X thuận (Forward Projection) trong dải góc giới hạn để tạo Sinogram
            sino = self.radon_op(phantom)

            # 3. Thêm nhiễu Poisson (mô phỏng số lượng photon tới detector theo định luật Beer-Lambert)
            ...
            # 4. Thêm nhiễu Gaussian (mô phỏng nhiễu mạch điện tử của cảm biến)
            if self.gaussian_level > 0:
                noise = float(self.gaussian_level) * torch.randn_like(sino_noisy)
                sino_noisy = sino_noisy + noise

            # 5. Tái tạo ảnh ban đầu bằng FBP từ sinogram góc giới hạn (sẽ xuất hiện artifacts do thiếu góc quét)
