#!/bin/bash
#SBATCH --job-name=test_dudotrans_la
#SBATCH --output=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_dudotrans_la/log/%j.out
#SBATCH --error=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_dudotrans_la/log/%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# SCRIPT ĐÁNH GIÁ (TEST BENCHMARK) MÔ HÌNH BASELINE DUDOTRANS (Dual-Domain Transformer)
...
echo "[INFO] Launching DuDoTrans Testing on Limited-Angle CT at $(date)"

cd /datastore/uittogether3/LuuTru/MinhPD
export PYTHONPATH="/datastore/uittogether3/LuuTru/MinhPD:${PYTHONPATH:-}"

# Xác định Checkpoint tốt nhất (ưu tiên epoch=38 đỉnh val_psnr=25.7300, sau đó đến last.ckpt)
CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/DuDoTrans/DuDoTrans_AAPM_LA_120deg_view64/epoch=38-val_psnr=25.7300.ckpt"
if [ ! -f "$CKPT_PATH" ]; then
    CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/DuDoTrans/DuDoTrans_AAPM_LA_120deg_view64/last.ckpt"
fi
echo "[INFO] Selected Checkpoint: $CKPT_PATH"

echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 1: DuDoTrans trên Cấu hình Chuẩn LA-120° (64 views)"
...
echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 2: DuDoTrans trên Cấu hình Khắc nghiệt LA-90° (64 views)"
echo "   (Đánh giá khả năng bù đắp góc khuyết mở rộng 270° của kiến trúc biến đổi đa miền)"
echo "================================================================================"
python -u baselines/DuDoTrans/test_dudotrans_la.py \
    --ckpt_path "$CKPT_PATH" \
    --cache_dir /datastore/uittogether3/LuuTru/MinhPD/dataset/aapm/limited_angle/ \
...
echo "[INFO] DuDoTrans Testing finished at $(date)"
