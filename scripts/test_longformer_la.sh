#!/bin/bash
#SBATCH --job-name=test_longformer_la
#SBATCH --output=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_longformer_la/log/%j.out
#SBATCH --error=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_longformer_la/log/%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# SCRIPT ĐÁNH GIÁ (TEST BENCHMARK) MÔ HÌNH BASELINE LEARN_LONGFORMER
...
echo "[INFO] Launching LEARN_Longformer Testing on Limited-Angle CT at $(date)"

cd /datastore/uittogether3/LuuTru/MinhPD
export PYTHONPATH="/datastore/uittogether3/LuuTru/MinhPD:${PYTHONPATH:-}"

# Xác định Checkpoint tốt nhất (ưu tiên epoch=45 đỉnh SOTA, sau đó đến last.ckpt)
CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/LEARN_Longformer/longformer_la-epoch=45-val_psnr=34.77-val_ssim=0.9383.ckpt"
if [ ! -f "$CKPT_PATH" ]; then
    CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/LEARN_Longformer/last.ckpt"
fi
echo "[INFO] Selected Checkpoint: $CKPT_PATH"

echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 1: LEARN_Longformer trên Cấu hình Chuẩn LA-120° (64 views)"
...
echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 2: LEARN_Longformer trên Cấu hình Khắc nghiệt LA-90° (64 views)"
echo "   (Đánh giá khả năng bù đắp góc khuyết mở rộng 270° của Sliding-Chunks Attention)"
echo "================================================================================"
python -u baselines/LEARN_Longformer/test_longformer_la.py \
    --checkpoint_path "$CKPT_PATH" \
    --cache_dir /datastore/uittogether3/LuuTru/MinhPD/dataset/limited_angle/ \
...
echo "[INFO] LEARN_Longformer Testing finished at $(date)"
