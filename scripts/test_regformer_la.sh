#!/bin/bash
#SBATCH --job-name=test_regformer_la
#SBATCH --output=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_regformer_la/log/%j.out
#SBATCH --error=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_regformer_la/log/%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# SCRIPT ĐÁNH GIÁ (TEST BENCHMARK) MÔ HÌNH BASELINE REGFORMER (Local CNN + Swin Transformer)
...
echo "[INFO] Launching RegFormer Testing on Limited-Angle CT at $(date)"

cd /datastore/uittogether3/LuuTru/MinhPD
export PYTHONPATH="/datastore/uittogether3/LuuTru/MinhPD:${PYTHONPATH:-}"

# Xác định Checkpoint tốt nhất (ưu tiên epoch=46 đỉnh val_psnr=36.0533, sau đó đến last.ckpt)
CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/RegFormer/RegFormer_AAPM_LA_120deg_view64/epoch=46-val_psnr=36.0533.ckpt"
if [ ! -f "$CKPT_PATH" ]; then
    CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/RegFormer/RegFormer_AAPM_LA_120deg_view64/last.ckpt"
fi
echo "[INFO] Selected Checkpoint: $CKPT_PATH"

echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 1: RegFormer trên Cấu hình Chuẩn LA-120° (64 views)"
echo "   (Tập kiểm thử độc lập Patient L310: 214 lát cắt CT)"
echo "================================================================================"
python -u baselines/RegFormer/test_regformer_la.py \
    --ckpt_path "$CKPT_PATH" \
...
echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 2: RegFormer trên Cấu hình Khắc nghiệt LA-90° (64 views)"
echo "   (Đánh giá khả năng bù đắp góc khuyết mở rộng 270° của cơ chế điều hòa Local-Nonlocal)"
echo "================================================================================"
python -u baselines/RegFormer/test_regformer_la.py \
    --ckpt_path "$CKPT_PATH" \
    --cache_dir /datastore/uittogether3/LuuTru/MinhPD/dataset/aapm/limited_angle/ \
...
echo "[INFO] RegFormer Testing finished at $(date)"
