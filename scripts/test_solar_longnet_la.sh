#!/bin/bash
#SBATCH --job-name=test_solar_longnet_la
#SBATCH --output=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_solar_longnet_la/log/%j.out
#SBATCH --error=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/test_solar_longnet_la/log/%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# SCRIPT ĐÁNH GIÁ (TEST BENCHMARK) MÔ HÌNH ĐỀ XUẤT SOLAR_LONGNET
...
echo "[INFO] Launching SOLAR_LongNet Testing on Limited-Angle CT at $(date)"

cd /datastore/uittogether3/LuuTru/MinhPD
export PYTHONPATH="/datastore/uittogether3/LuuTru/MinhPD:${PYTHONPATH:-}"

# Xác định Checkpoint tốt nhất (ưu tiên epoch=29 tốt nhất, sau đó đến last.ckpt)
CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/SOLAR_LongNet/solar_longnet_la-epoch=29-val_psnr=32.26-val_ssim=0.8935.ckpt"
if [ ! -f "$CKPT_PATH" ]; then
    CKPT_PATH="/datastore/uittogether3/LuuTru/MinhPD/saved_models/SOLAR_LongNet/last.ckpt"
fi
echo "[INFO] Selected Checkpoint: $CKPT_PATH"

echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 1: SOLAR_LongNet trên Cấu hình Chuẩn LA-120° (64 views)"
echo "   (Tập kiểm thử độc lập Patient L310: 214 lát cắt CT)"
echo "================================================================================"
python -u baselines/SOLAR_LongNet/test_solar_longnet_la.py \
...
echo "================================================================================"
echo "🎯 ĐÁNH GIÁ 2: SOLAR_LongNet trên Cấu hình Khắc nghiệt LA-90° (64 views)"
echo "   (Đánh giá khả năng bù đắp góc khuyết mở rộng 270° của Dilated Attention & Newton-CG)"
echo "================================================================================"
python -u baselines/SOLAR_LongNet/test_solar_longnet_la.py \
    --checkpoint_path "$CKPT_PATH" \
    --dataset_dir /datastore/uittogether3/LuuTru/MinhPD/dataset/limited_angle/ \
...
echo "[INFO] SOLAR_LongNet Testing finished at $(date)"
