#!/bin/bash
#SBATCH --job-name=tr_solar__deeplesion
#SBATCH --output=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/train_solar_longformer_deeplesion/log/%j.out
#SBATCH --error=/datastore/uittogether3/LuuTru/MinhPD/scripts/output/train_solar_longformer_deeplesion/log/%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# ==============================================================================
# SCRIPT HUẤN LUYỆN: SOLAR_Longformer trên NIH DeepLesion CT (Limited-Angle CT)
...
# ================= RUN TRAINING =================
echo "[INFO] Launching SOLAR_Longformer Training on NIH DeepLesion CT at $(date)"

cd /datastore/uittogether3/LuuTru/MinhPD
export PYTHONPATH="/datastore/uittogether3/LuuTru/MinhPD:${PYTHONPATH:-}"

RESUME_CKPT="/datastore/uittogether3/LuuTru/MinhPD/saved_models/deeplesion/SOLAR_Longformer/last.ckpt"
EXTRA_ARGS=""
...
python -u baselines/SOLAR_Longformer/train_solar_longformer_la.py \
    --dataset_type deeplesion \
    --dataset_dir /datastore/uittogether3/LuuTru/MinhPD/dataset/nih_deep_lesion/limited_angle/ \
    --output_dir /datastore/uittogether3/LuuTru/MinhPD/saved_models/deeplesion/SOLAR_Longformer/ \
    ...
    --max_epochs 50 \
    --use_precomputed \
    --n_iterations 8 --cg_iters 4 --lr 1e-4 --final_lr 1e-5 --window_size 2 --num_global 50 \
    $EXTRA_ARGS

echo "[INFO] SOLAR_Longformer Training on NIH DeepLesion CT finished at $(date)"
